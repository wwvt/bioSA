...
# LHD Design
# Created on 4/19/2016, Last update on 2/9/2017
# 
...
X_full = do.call(rbind, replicate(10, X, simplify=FALSE));   # replicate the design points for 3 times
...
# Focus on weeks 2 to 6, with 14 time steps a segment, leading to 10 time segments in total
# {  
# T1= 29:42   
# T2= 43:56  
# T3= 57:70   
...
time_seg = switch(k, 29:42, 43:56, 57:70, 71:84, 85:98, 99:112, 113:126, 127:140, 141:154, 155:168);
...
dim(t_full) = c(128*10,1);
dim(y_mean) = c(128*10,1);

# y_mean_max = pmax(y_mean, matrix(1,128*3,1));  # transform the counts to reduce the output variation
y_mean_max =  y_mean + matrix(1,128*10,1);  # transform the counts to reduce the output variation
...
save.image('M0_LP_norm_log_T2S10.Rdata');  # save the fitted SK metamodel for one output type, remember to rename it for different outputs differently
